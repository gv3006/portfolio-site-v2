      ...
      <div className="mt-16 divide-y divide-border border-t border-border">
        {steps.map((step, index) => (
          <Reveal key={step.number} delay={index * 80}>
            <article className="grid grid-cols-1 md:grid-cols-12 gap-4 py-8 group transition-colors hover:bg-foreground/[0.02] -mx-6 px-6">
              <div className="md:col-span-1 font-mono text-xs tracking-widest text-foreground/40">{step.number}</div>
              <div className="md:col-span-7">
                <h3 className="font-mono text-lg tracking-[0.2em] uppercase">{step.title}</h3>
                <p className="mt-2 text-sm leading-relaxed text-foreground/50 text-pretty">{step.description}</p>
              </div>
              <div className="md:col-span-4 md:text-right">
                <span className="inline-block font-mono text-xs tracking-widest uppercase text-foreground/60 border border-border rounded-full px-4 py-1">
                  {step.label}
                </span>
              </div>
            </article>
